...
public interface WareHouseRepository extends JpaRepository<WareHouse, Long> {
    ...
    @Query("SELECT w FROM WareHouse w left join Warehouse_product wp ON wp.warehouser_id = w.id LEFT JOIN Product p ON wp.product_id = p.id where w.is_active IN (:isActive)" +
